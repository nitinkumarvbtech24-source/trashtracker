...
def prepare_combined_dataset(
    ...
    for orig_split in ["train", "valid"]:
        ...
        if os.path.exists(img_dir):
            for img_name in os.listdir(img_dir):
                ...
                global_cls = 6 if has_garbage else 4
                labels.append(f"{global_cls} 0.500000 0.500000 1.000000 1.000000")
                label_text = "\n".join(labels) + "\n"
    ...
    aiq_count = 0
    if os.path.exists(aiq_photos_path):
        for root_dir, _, files in os.walk(aiq_photos_path):
            for f in files:
                if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.webp')):
                    ...
                    if os.path.exists(pseudo_lbl_path):
                        ...
                        label_text = "6 0.500000 0.500000 1.000000 1.000000\n0 0.500000 0.500000 0.400000 0.400000\n"

                    real_items.append((img_path, label_text, f"aiq_{f}", True))
                    aiq_count += 1

    print(f"Total real items collected: {len(real_items)} (including {aiq_count} AIQ photos)")
    ...
    for src_img, lbl_text, out_name, is_aiq in train_real:
        ...
        if enable_augmentation and is_aiq:
            # Pick a subset of advanced augmentations per image to avoid explosion while ensuring diversity
    ...
    if os.path.exists(synthetic_dir):
        ...
        for syn_path in syn_files:
            ...
            syn_label = "6 0.500000 0.500000 1.000000 1.000000\n0 0.500000 0.500000 0.500000 0.500000\n"
            with open(dst_lbl, "w") as f:
                f.write(syn_label)
            syn_count += 1
...
nc: 7
names: ['Garbage', 'Pothole', 'Debris', 'Abandoned Vehicle', 'Clean Road', 'Slightly Dirty Road', 'Very Dirty Road']
"""
    yaml_path = os.path.join("garbage_ai model", "combined_data.yaml")
    with open(yaml_path, "w") as f:
        f.write(yaml_content)

    print(f"Saved dataset yaml config to: {yaml_path}")
    print("Advanced dataset augmentation complete!\n")
