...
@app.route('/health_api/detections/<path:filename>', methods=['GET'])
@cross_origin()
def proxy_health_images(filename):
    try:
        resp = requests.get(f'http://127.0.0.1:5002/detections/{filename}')
        return Response(resp.content, content_type=resp.headers.get('Content-Type', 'image/jpeg'))
    except Exception as e:
        return jsonify({"error": str(e)}), 500
