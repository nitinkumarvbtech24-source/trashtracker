...
def draw_overlays(cv_frame, road_status, road_conf, detections):
    ...
    status_colors = {
        ...
        "Not a Road": (128, 128, 128),      # Grey
        "Not_a_Road": (128, 128, 128)
    }
...
@app.route('/images/<road_class>/<filename>')
@cross_origin()
def serve_image(road_class, filename):
    """Serve images with CORS headers applied by flask_cors"""
    return send_from_directory(os.path.join(IMAGE_DIR, road_class), filename)
...
def process_frame_route():
    try:
        ...
        if road_class not in ["Clean", "Not_a_Road"]:
        ...
        filepath = os.path.join(IMAGE_DIR, road_class, filename)
        ...
        if len(recent_snapshots) > 50:
            ...
            try:
                os.remove(os.path.join(IMAGE_DIR, oldest["class"], oldest["filename"]))
            except:
                pass
