...
def process_frame(cv_frame, classifier, detector, clip_model, clip_processor, device, conf_threshold=0.25):
    ...
    # 1. Stage 1: Road Classification (Keras Model)
    ...
    road_class = ROAD_CLASSES[class_id] if class_id < len(ROAD_CLASSES) else "Clean"
    ...
    # 2. Stage 2: Garbage Detection (if not Clean and not Not_a_Road)
    detections_summary = []
    if road_class not in ["Clean", "CleanRoad", "Not_a_Road"]:
