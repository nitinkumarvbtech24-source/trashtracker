...
CLASSIFIER_PATH = os.path.join(BASE_DIR, "outputs", "best_model.keras") if os.path.exists(os.path.join(BASE_DIR, "outputs", "best_model.keras")) else ("garbage_ai model/outputs/best_model.keras" if os.path.exists("garbage_ai model/outputs/best_model.keras") else "outputs/best_model.keras")
DETECTOR_PATH = os.path.join(BASE_DIR, "weights", "garbage_detector.pt") if os.path.exists(os.path.join(BASE_DIR, "weights", "garbage_detector.pt")) else ("garbage_ai model/weights/garbage_detector.pt" if os.path.exists("garbage_ai model/weights/garbage_detector.pt") else "weights/garbage_detector.pt")

CLIP_MODEL_NAME = "openai/clip-vit-base-patch32"

# 4-Class Keras Classifier Labels (alphabetical dataset folder mapping)
ROAD_CLASSES = ["Clean", "Not_a_Road", "Slightly_Dirty", "Very_Dirty"]
...
def run_pipeline(image_path, classifier, detector, clip_model, clip_processor, device, output_dir="output"):
    ...
    if class_id < len(ROAD_CLASSES):
        road_class = ROAD_CLASSES[class_id]
    else:
        road_class = "Clean"
    ...
    # If not a road
    if road_class == "Not_a_Road":
        print("Classifier detected: Image is not a road.")
        cv2.putText(cv_image, "Not a Road", (20, 40),
    ...
    if road_class == "Clean" or road_class == "CleanRoad":
    ...
    det_results = detector.predict(source=pil_image, conf=0.45, verbose=False)
